...
**Nested PyInstaller helpers.** `engine.app` and `globus_cli.app` are COLLECT folders (not real `.app` bundles) copied into `SlideRelabeler.app/Contents/Resources`. Apple notarization rejects the zip if those Mach-Os are unsigned or ad-hoc. When `CSC_LINK` is set, Packager `afterCopyExtraResources` runs [`scripts/sign-pyinstaller-helpers.mjs`](../scripts/sign-pyinstaller-helpers.mjs) on the **packaged** helper trees (hardened runtime + timestamp + [`scripts/entitlements.mac.plist`](../scripts/entitlements.mac.plist)). `osxSign.ignore` then skips those paths so Forge does not re-sign them when signing the outer app. Notarize the outer app once — do **not** notarize the helpers separately.
