...
| **Profiles** | ConfigSection → Callout → SectionPanel → SettingHeader (description embeds Import text button) + list-box multi-select (active/modified row status) + name-editor panel + `cfg-panel-actions` (Activate / Save as / Save / Rename / Export / Delete). Deep link: `config-profiles`. |
