...
/**
 * Default DSA server URL with blur/button reachability check.
 */
export default function DsaDefaultUrlField({ disabled = false, value = '', onChange }) {
  ...
  const statusClass = [
    'output-delivery-section__dsa-url-status',
    status === 'valid' ? 'output-delivery-section__dsa-url-status--valid' : '',
    status === 'invalid' ? 'output-delivery-section__dsa-url-status--invalid' : '',
    status === 'checking' ? 'output-delivery-section__dsa-url-status--checking' : '',
  ].filter(Boolean).join(' ');

  return (
    <div className="output-delivery-section__dsa-url-field">
      <div className="output-delivery-section__dsa-url-label-row">
        <span className="output-delivery-section__dsa-url-label">Default server address</span>
      ...
      <div className="output-delivery-section__dsa-url-input-row">
        ...
        <Button
          ...
          extra_class_name="output-delivery-section__dsa-url-check-btn"
        />
      </div>
    </div>
  );
