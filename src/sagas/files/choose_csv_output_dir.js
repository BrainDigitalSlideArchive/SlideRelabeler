...
export default function* choose_csv_output_dir () {
  while(true) {
    const action = yield take(files_actions.CHOOSE_CSV_OUTPUT_DIR);
    ...
    if (folder && typeof(folder) === "string" || !'error' in folder) {
      yield put({type: files_actions.SET_CSV_OUTPUT_DIR, payload: folder});
    } else {
